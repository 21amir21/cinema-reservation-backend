...
<head>
  ...
  <script>
    function getSelectedSeats(id, theatreID) {
      ...
      if (selectedSeatsIds.length) {
        window.location.href = "/../views/Payment.php?id=" + id + "&seats=" + JSON.stringify(selectedSeatsIds) +
          "&theatreID=" + theatreID;
      }
    }
  </script>
...
<?php
  ...
  $sql = "SELECT theatreId FROM showtime WHERE movieId = $id";

  $row = $conn->query($sql);
  $theatres = mysqli_fetch_assoc($row);
  $theatreId = $theatres['theatreId'];

  $bookedSeats = gettingSeatsFromDatabase($theatreId);
  $seatNumber = 0;
?>
...
<div id="main-container">
  ...
  <div style="width: 700px; align-self: center; justify-content: space-between">
    <a href="../views/movie_details.php?id=<?= $id ?>">
      <button type="button" class="book" style="float: left">
        Cancel
      </button>
    </a>

    <a href="#" onclick="getSelectedSeats(<?= $id ?>, <?= $theatreId ?>)">
